...
export default function PracticeClient({ stageId }: Props) {
  ...
  if (!stage) {
    return <div style={{ padding: 16, color: '#e5e5e5' }}>단계를 찾을 수 없습니다.</div>;
  }
  ...
  if (!canAccessStage(stageId)) {
    ...
    if (stageId > 18) {
      return (
        <div style={{ minHeight: '100vh', background: '#030712', color: '#e5e5e5', padding: 16, maxWidth: 512, margin: '0 auto' }}>
          <header style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
            <button
              onClick={() => router.push('/journey')}
              style={{ color: '#9ca3af', background: 'none', border: 'none', cursor: 'pointer', fontSize: 18 }}
            >
              &larr;
            </button>
            <div>
              <div style={{ fontSize: 14, color: '#9ca3af' }}>{stage?.blockIcon} {stage?.block}</div>
    ...
    return (
      <div style={{ minHeight: '100vh', background: '#030712', color: '#e5e5e5', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
        <div style={{ textAlign: 'center' }}>
          <p style={{ fontSize: 36 }}>--</p>
          <p style={{ marginTop: 16, color: '#9ca3af' }}>이전 단계를 먼저 완료해주세요.</p>
          <button
            onClick={() => router.push('/journey')}
            style={{ marginTop: 16, color: '#818cf8', textDecoration: 'underline', background: 'none', border: 'none', cursor: 'pointer' }}
          >
            여정으로 돌아가기
          </button>
        </div>
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#030712', color: '#e5e5e5', padding: 24, maxWidth: 672, margin: '0 auto' }}>
      {/* 헤더 */}
      <header style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
        ...
        <div>
          <div style={{ fontSize: 14, color: '#9ca3af' }}>{stage.block}</div>
