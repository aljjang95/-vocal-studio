...
export default function SessionReportPanel({ report, tensionHistory }: Props) {
  ...
  return (
    <div style={{ marginTop: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
      {/* 헤더 */}
      <div style={{ padding: 16, borderRadius: 8, background: 'rgba(17,24,39,0.7)', border: '1px solid #374151' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
          <h3 style={{ fontSize: 18, fontWeight: 700, color: '#e5e5e5', margin: 0 }}>세션 리포트</h3>
          <TTSButton text={reportText} size="md" label="리포트 듣기" />
        </div>
        <p style={{ fontSize: 14, color: '#d1d5db', margin: 0 }}>{report.summary}</p>
      </div>

      {/* 긴장도 미니 그래프 */}
      {tensionHistory.length > 0 && (
        <div style={{ padding: 12, borderRadius: 8, background: 'rgba(17,24,39,0.5)', border: '1px solid #1f2937' }}>
          <p style={{ fontSize: 12, color: '#6b7280', marginBottom: 8, margin: '0 0 8px' }}>긴장도 추이</p>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 }}>
            <span style={{ fontSize: 10, color: '#4b5563' }}>시작</span>
            <span style={{ fontSize: 10, color: '#4b5563' }}>종료</span>
          </div>
        </div>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
        <div style={{ padding: 8, borderRadius: 6, background: 'rgba(17,24,39,0.3)', textAlign: 'center' }}>
          <div style={{ fontSize: 18, fontWeight: 700, color: '#e5e5e5' }}>{stats.chunk_count}</div>
          <div style={{ fontSize: 10, color: '#6b7280' }}>분석 횟수</div>
        </div>
        <div style={{ padding: 8, borderRadius: 6, background: 'rgba(17,24,39,0.3)', textAlign: 'center' }}>
          <div style={{ fontSize: 18, fontWeight: 700, color: '#e5e5e5' }}>{stats.avg_tension}</div>
          <div style={{ fontSize: 10, color: '#6b7280' }}>평균 긴장도</div>
        </div>
        <div style={{ padding: 8, borderRadius: 6, background: 'rgba(17,24,39,0.3)', textAlign: 'center' }}>
          <div style={{ fontSize: 18, fontWeight: 700, color: '#e5e5e5' }}>{stats.tension_events}</div>
          <div style={{ fontSize: 10, color: '#6b7280' }}>긴장 감지</div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {report.improvements && (
          <div style={{ padding: 12, borderRadius: 8, background: 'rgba(6,47,26,0.2)', border: '1px solid rgba(6,78,59,0.3)' }}>
            <p style={{ fontSize: 12, color: '#34d399', marginBottom: 4, margin: '0 0 4px' }}>잘된 점</p>
            <p style={{ fontSize: 14, color: '#a7f3d0', margin: 0 }}>{report.improvements}</p>
          </div>
        )}
        {report.focus_area && (
          <div style={{ padding: 12, borderRadius: 8, background: 'rgba(43,25,0,0.2)', border: '1px solid rgba(92,50,0,0.3)' }}>
            <p style={{ fontSize: 12, color: '#fbbf24', marginBottom: 4, margin: '0 0 4px' }}>다음 집중 포인트</p>
            <p style={{ fontSize: 14, color: '#fde68a', margin: 0 }}>{report.focus_area}</p>
          </div>
        )}
        {report.exercise && (
          <div style={{ padding: 12, borderRadius: 8, background: 'rgba(7,16,43,0.2)', border: '1px solid rgba(30,58,138,0.3)' }}>
            <p style={{ fontSize: 12, color: '#60a5fa', marginBottom: 4, margin: '0 0 4px' }}>추천 연습</p>
            <p style={{ fontSize: 14, color: '#bfdbfe', margin: 0 }}>{report.exercise}</p>
          </div>
        )}
      </div>

      {/* 격려 */}
      <p style={{ textAlign: 'center', fontSize: 14, color: '#a5b4fc', padding: '8px 0', margin: 0 }}>{report.encouragement}</p>
    </div>
  );
}
